...
{
// ------ My code ------------
//std::cerr << data.getName()[1].toUri() << std::endl;

if(!data.getName()[0].toUri().compare("ndn"))
{

//std::cerr << " before " << data.getName().toUri() << std::endl;
uint64_t my_nextSegmentNo =  data.getName()[-1].toSegment();
my_nextSegmentNo = my_nextSegmentNo +11 ;

//std::cerr << " after "<< std::endl;

if(my_nextSegmentNo <= data.getFinalBlockId().toSegment())
{
...
//               boost::thread thread1(&ndn::chunks::ndnChunks::startChunk, & nchunks,newName);
        //      thread1.join();
